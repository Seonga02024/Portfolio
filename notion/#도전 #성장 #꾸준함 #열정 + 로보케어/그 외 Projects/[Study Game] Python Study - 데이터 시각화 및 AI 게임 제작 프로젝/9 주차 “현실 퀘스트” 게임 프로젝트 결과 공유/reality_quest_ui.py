...
# --- 게임 설정 ---
QUEST_ITEMS = ["cup", "book", "keyboard", "bottle", "cell phone", "mouse", "remote", "chair", "laptop", "clock"]
...
def main():
    ...
    stage_img_path = "C:/Users/robocare/Desktop/stageImg1.png"
    stage_img = cv2.imread(stage_img_path, cv2.IMREAD_COLOR)
    if stage_img is None:
        print(f"경고: 스테이지 이미지를 찾을 수 없습니다 ({stage_img_path}).")
        stage_img = np.ones((CAM_SIZE, CAM_SIZE, 3), dtype=np.uint8) * 100
    else:
        stage_img = cv2.resize(stage_img, (CAM_SIZE, CAM_SIZE))

    teacher_img_path = "C:/Users/robocare/Desktop/teacher.png"
    teacher_img = cv2.imread(teacher_img_path, cv2.IMREAD_COLOR)
    ...
    bg_img_path = "C:/Users/robocare/Desktop/backgroundImg.png"
    bg_img = cv2.imread(bg_img_path, cv2.IMREAD_COLOR)
    if bg_img is None:
        print(f"경고: 배경 이미지를 찾을 수 없습니다 ({bg_img_path}). 흰색 배경으로 대체합니다.")
        ...
        final_bg_texture = cv2.addWeighted(bg_img, 0.3, white_canvas, 0.7, 0)
    # --- 여기까지 ---
    ...
    while True:
        ...
        if game_state == STATE_TITLE:
            ...
            background = draw_multiline_korean_text(background, story_text, (WIN_WIDTH//2, 280), 22, (0,0,0))
            ...
            if pre_gameplay_flow == 0:
                background = draw_korean_text(background, "제자야... 무사히 잘 있는 거 맞느냐?", (WIN_WIDTH//2, TEXT_Y), 22, (0,0,0))
                background = draw_korean_text(background, "나한테 얼굴을 보여보거라", (WIN_WIDTH//2, TEXT_Y + 30), 22, (0,0,0))
                ...
                background = draw_korean_text(background, "확인되었구나. 클릭하여 계속하거라.", (WIN_WIDTH//2, TEXT_Y), 22, (0,0,0))
                if mouse_clicked:
                    pre_gameplay_flow = 2
                    dialogue_start_time = time.time()
                    mouse_clicked = False
            elif pre_gameplay_flow == 2:
                story_text = "내가 확인해보니 총 10가지의 물건을\n찾아야 하는 것 같구나...\n내가 오래 힘을 유지할 수가 없어\n빠르게 해내야할 것 같구나...\n만약 다 찾지 못한다면\n너는 다시 이 곳에 돌아올 수 없단다\n명심하렴...!"
                background = draw_multiline_korean_text(background, story_text, (WIN_WIDTH//2, TEXT_Y - 20), 20, (0,0,0))
                if time.time() - dialogue_start_time > 7:
            ...
            IMG_Y = 50
            QUEST_TEXT_Y = IMG_Y + CAM_SIZE + 100
            TIME_TEXT_Y = QUEST_TEXT_Y + 80
            DETECT_TEXT_Y = TIME_TEXT_Y + 80
            CAM_Y = DETECT_TEXT_Y + 80
            ...
            background[IMG_Y:IMG_Y+CAM_SIZE, CAM_X_OFFSET:CAM_X_OFFSET+CAM_SIZE] = stage_img
            
            quest_item = QUEST_ITEMS[current_quest_index]
            background = draw_korean_text(background, f"요구하는 물체: {quest_item}", (WIN_WIDTH//2, QUEST_TEXT_Y), 25, (0, 0, 0))
            background = draw_korean_text(background, f"제한 시간: {int(time_remaining)}초", (WIN_WIDTH//2, TIME_TEXT_Y), 25, (255, 0, 0))
            background = draw_korean_text(background, f"인식된 물체: {last_detected_object}", (WIN_WIDTH//2, DETECT_TEXT_Y), 25, (0, 0, 0))
            ...
            if found_quest_object:
                ...
                if time.time() - quest_completed_time > 2:
                    ...
                    quest_completed_time = 0
                    quest_start_time = time.time()
                    if current_quest_index >= len(QUEST_ITEMS): game_state = STATE_GAMEOVER
            ...
            background = draw_multiline_korean_text(background, end_text, (WIN_WIDTH//2, 300), 22, (0,0,0))
